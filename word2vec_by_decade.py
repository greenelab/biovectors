...
def similarity_scores(model, pairs, year):
    ...
    similarity_scores_df.to_csv(f"outputs/decades/similarity_scores_{str(year)}-{str(year+9)}.tsv", sep="\t", index=False)
    print(f"Similarity scores written to file")
...
def dummy(scores_filename, year):
    ...
    print(f"Creating dummy classifer")
    ...
    print(f"Dummy scores written to file")
...
if __name__ == "__main__":
    ...
    dates_df = pd.read_csv(
        "inputs/pmid_to_pub_date.tsv.xz",
        compression="xz",
        sep="\t"
    )
    ...
    for year in years:
        ...
        pmids = set(
            dates_df.loc[
                dates_df["pub_date"].between(year, year+9),
                "pmid"
            ].tolist()
        )
        if len(pmids) > 0:
            ...
            sentences = Sentences(
                os.path.join(base, "inputs/bioconcepts2pubtatorcentral.gz"),
                pmids
            )
            ...
            if count > 0:
                print(f"At least one abstract available")
                model = word2vec(sentences, year)
                similarity_scores(model, pairs, year)
                dummy(
                    os.path.join(base, f"outputs/decades/similarity_scores_{str(year)}-{str(year+9)}.tsv"),
                    year
                )
            else:
                print(f"No abstracts available")
        else:
            print(f"No PMIDs from this time period")
